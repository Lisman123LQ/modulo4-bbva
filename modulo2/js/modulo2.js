...
async function guardarRegistro(){
  ...
  if(!numero){
    alert('Ingresa el número PLIN.');
    return false;
  }

  if(!fotos.qr){
    alert('La foto del QR PLIN es obligatoria.');
    return false;
  }
  ...
  if(registros.some(r=>String(r.numero)===numero)){
    alert('Este número PLIN ya aparece en los registros cargados.');
    return false;
  }
  ...
  try{
    ...
    if(error){
      ...
      if(error.code==='23505'){
        alert('❌ Ese número PLIN ya está registrado por otro usuario.');
      }else{
...
async function exportarExcel(){
  ...
  try{
    ...
    workbook.creator='Módulo 4 PLIN';
    workbook.created=new Date();

    const ws=workbook.addWorksheet('MODULO 2');
    ...
    ws.columns=[
      ...
      {header:'NÚMERO PLIN',key:'numero',width:17},
      {header:'MÓDULO',key:'sino',width:10},
      {header:'FECHA',key:'fecha',width:20},
      {header:'FOTO QR PLIN',key:'qr',width:22}
    ];
...
function crearCopiaSeguridad(){
  ...
  try{
    ...
    enlace.download=`BACKUP_MODULO4_PLIN_${nombreFecha.replace(' ','')}.json`;
